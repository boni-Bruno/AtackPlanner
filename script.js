...
(function () {
  ...
  /* Unidades habilitadas no mundo (lista de strings do game_data.units) */
  ...
  /* ══════════════════════════════════════════════════════
     ...
     Fórmula TW: tempo = distância × base_speed × unit_speed / world_speed
  ══════════════════════════════════════════════════════ */
  var ALL_TROOPS = [
  ...
  /* Filtra apenas tropas do mundo atual; milícia (speed=0) sempre excluída do planejamento */
  ...
  /* ══════════════════════════════════════════════════════
     BUSCA CONFIGURAÇÃO DO MUNDO VIA API
     /interface.php?func=get_config  →  XML com speed e unit_speed
  ══════════════════════════════════════════════════════ */
      ...
      '<div class="ap-pnl on" id="ap-p-plan">' +
        ...
        '<div class="ap-sec">' +
          ...
          '<div class="ap-sb">' +
            ...
            '<div id="ap-res">' +
              '<div class="ap-rg">' +
                '<div class="ap-rc"><span class="v" id="ap-r1">—</span><span class="l">Distância (campos)</span></div>' +
  ...
  /* ══════════════════════════════════════════════════════
     CÁLCULO DE TEMPO
     Fórmula TW: tempo (min) = distância × base_speed × unit_speed / world_speed
  ══════════════════════════════════════════════════════ */
  ...
  function recalc() {
    ...
    var tmin = calcTroopTime(d, selTroop.speed);
    var tms  = tmin * 60000;
  ...
  window.AP = {
    ...
    csv: function () {
      ...
      a.href = url; a.download = 'attack_planner_' + WORLD_ID + '.tsv'; a.click();
      URL.revokeObjectURL(url);
    }
  };

  /* ══════════════════════════════════════════════════════
     INJETAR CSS + MOSTRAR LOADING + BUSCAR CONFIG + ABRIR
  ══════════════════════════════════════════════════════ */
  var styleEl = document.getElementById('ap-css') || document.createElement('style');
  styleEl.id = 'ap-css'; styleEl.textContent = CSS;
  document.head.appendChild(styleEl);

  /* Placeholder enquanto busca config */
